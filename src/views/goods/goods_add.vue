      ...
      this.$refs.GoodsRef.validate((valid) => {
        ...
        this.GoodsCount(this.queryGoods, (res) => {
          ...
          var params = {
            ...
            goods_img_arr: this.queryGoods.goods_img_arr.join(',')||'1231',
            goods_keyword: this.queryGoods.goods_keyword.join('-'),
            contnt: this.queryGoods.contnt,
            goods_small_img: this.queryGoods.goods_small_img||'123',
          ...
          // 发送请求
          this.$http.post(this.API.BASE_API_GOODS_ADD, params).then((res) => {
            if (res.data.status !== 201)
              return this.$message.error(res.data.msg)
            this.$message.success(res.data.msg)
            this.Storage.local.remove('editor')
            this.$router.push('/goods')
            this.$store.commit('editState',4)
            this.queryGoods = {
              goods_name: null,
              goods_price: 1,
              goods_discount_price: 1,
